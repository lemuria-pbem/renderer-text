...
<div class="fixed-top vh-100">
	...
	<nav id="navbar" class="navbar navbar-light bg-light collapse pt-0">
						...
						<?php if ($census->has($region->Id())): ?>
							<?php if ($region->Landscape() instanceof Navigable): ?>
								<?php if ($isMonster || !$region->Fleet()->isEmpty()): ?>
									<a class="navigable nav-link pb-0" href="#<?= id($region) ?>"><?= $region->Name() ?></a>
								<?php endif ?>
								<?php foreach (View::sortedFleet($region) as $vessel): ?>
									<a class="vessel nav-link ml-3 py-0" href="#<?= id($vessel) ?>"><?= $vessel->Name() ?></a>
								<?php endforeach ?>
							<?php else: ?>
								<a class="location <?= strtolower(getClass($region->Landscape())) ?> nav-link pb-0" href="#<?= id($region) ?>"><?= $region->Name() ?></a>
